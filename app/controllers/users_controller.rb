class UsersController < ApplicationController
  ...
  before_action :not_admin_user, only: [:index, :new, :create, :destroy, :reset, :reset_password, :search] # restrict
  ...
  def update
    if current_user.user_role.name == 'admin' || @user.authenticate(params[:user][:password_previous])
  ...
  def update_password
    if @user.authenticate(params[:password_previous])
      if @user.update_attributes(password: params[:password], password_confirmation: params[:password_confirmation])
        redirect_to home_url, notice: "User password updated"
      else
  ...
  def editing_self
    if current_user.user_role.name != 'admin' && current_user.id != params[:id].to_i
